...
def build_analysis_overview_html(
    dfh: pd.DataFrame,
    ...
    """
    Costruisce una sintesi HTML del rapporto tra portafoglio e mercati.

    La funzione produce testo pronto per essere renderizzato nella UI,
    aggregando scenario recente, leadership per macro-categoria, peso
    dominante e indicatori di rischio/drawdown.
    """
    from persistence.storage import macro_cat
    from core.formatting import fmt_pct_it

    rows = []
    if dfh is not None and not dfh.empty and len(dfh) >= 2:
        lookback = min(21, len(dfh) - 1)
        cur_val = float(dfh["Valore"].iloc[-1])
        prev_val = float(dfh["Valore"].iloc[-1 - lookback])
        ret = (cur_val / prev_val - 1) if prev_val else np.nan
        direction = (
            "migliorando" if pd.notna(ret) and ret > 0.01
            else ("indebolendosi" if pd.notna(ret) and ret < -0.01 else "muovendosi lateralmente")
        )
        rows.append((
            "Scenario",
            f"Nel tratto più recente osservabile il portafoglio sta <b>{direction}</b>, "
            f"con una variazione di circa <b>{fmt_pct_it(ret, 2, signed=True)}</b> "
            f"rispetto a {lookback} rilevazioni fa."
        ))
    if cat_index is not None and not cat_index.empty and len(cat_index) >= 2:
        lookback = min(21, len(cat_index) - 1)
        perf = []
        for cat in cat_index.columns:
            prev = float(cat_index[cat].iloc[-1 - lookback])
            cur = float(cat_index[cat].iloc[-1])
            perf.append((cat, (cur / prev - 1) if prev else np.nan))
        perf = [(c, v) for c, v in perf if pd.notna(v)]
        if perf:
            perf_sorted = sorted(perf, key=lambda x: x[1], reverse=True)
            best_cat, best_ret = perf_sorted[0]
            worst_cat, worst_ret = perf_sorted[-1]
            if best_cat == worst_cat:
                rows.append(("Mercati", "La dinamica di mercato incorporata dal portafoglio appare piuttosto uniforme tra le macro-aree presidiate."))
            else:
                rows.append((
                    "Mercati",
                    f"La spinta migliore arriva da <b>{best_cat}</b> ({fmt_pct_it(best_ret, 2, signed=True)}), "
                    f"mentre l'area relativamente più debole è <b>{worst_cat}</b> ({fmt_pct_it(worst_ret, 2, signed=True)})."
                ))
    if da_frame is not None and not da_frame.empty:
        tmp = da_frame.copy()
        tmp["Categoria"] = tmp["Tipo"].apply(macro_cat)
        by_cat = tmp.groupby("Categoria")["Controvalore"].sum().sort_values(ascending=False)
        if not by_cat.empty:
            lead_cat = by_cat.index[0]
            lead_share = by_cat.iloc[0] / by_cat.sum() if by_cat.sum() else 0
            rows.append((
                "Peso",
                f"La lettura complessiva resta influenzata soprattutto dalla componente <b>{lead_cat}</b>, "
                f"che pesa per circa <b>{fmt_pct_it(lead_share, 2)}</b> sul valore di mercato totale."
            ))
    if dfstats is not None and not dfstats.empty:
        vol_data = dfstats.dropna(subset=["Volatilità Ann."])
        dd_data = dfstats.dropna(subset=["Max Drawdown"])
        if not vol_data.empty:
            high_vol = vol_data.sort_values("Volatilità Ann.", ascending=False).iloc[0]
            rows.append(("Rischio", f"Lo strumento con profilo più instabile è <b>{high_vol['Ticker']}</b>, con volatilità annua pari a <b>{fmt_pct_it(high_vol['Volatilità Ann.'], 2)}</b>."))
        if not dd_data.empty:
            deep_dd = dd_data.sort_values("Max Drawdown").iloc[0]
            rows.append(("Drawdown", f"La flessione massima più profonda nel campione riguarda <b>{deep_dd['Ticker']}</b>, con drawdown di <b>{fmt_pct_it(deep_dd['Max Drawdown'], 2)}</b>."))
    if not rows:
        return "Dati ancora insufficienti per una lettura sintetica del rapporto tra portafoglio e mercati."
    lines = [f'<div class="metric-guide-line"><span><b>{key}</b> — {desc}</span></div>' for key, desc in rows]
    return '<div class="metric-guide">' + ''.join(lines) + '</div>'
