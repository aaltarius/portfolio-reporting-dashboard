...
def build_accumuli_overview_chart(summary: pd.DataFrame) -> go.Figure:
    ...
    Ascissa = margine rispetto al PMC; ordinata = elasticità residua.
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    ...
                    "<br>Margine su PMC: %{x:.2%}"
                    "<br>Elasticità PMC: %{y:.2%}"
    ...
    fig.add_hline(y=0.08, line_dash="dot", line_color=hex_to_rgba(P["orange"], 0.65), opacity=0.7)
    fig.update_layout(
    ...
    fig.update_xaxes(
        title_text="Margine su PMC",
        tickformat=".1%",
        automargin=True,
        zeroline=True,
        range=_padded_axis_range(plot["margine_pmc"]),
    )
    fig.update_yaxes(
        title_text="Elasticità PMC",
        tickformat=".1%",
        automargin=True,
        zeroline=False,
